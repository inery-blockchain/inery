...
#include <inery/chain/block_log.hpp>
#include <inery/chain/fork_database.hpp>
#include <inery/chain/exceptions.hpp>

#include <inery/chain/account_object.hpp>
#include <inery/chain/code_object.hpp>
#include <inery/chain/block_summary_object.hpp>
#include <inery/chain/inery_contract.hpp>
#include <inery/chain/global_property_object.hpp>
#include <inery/chain/protocol_state_object.hpp>
#include <inery/chain/contract_table_objects.hpp>
#include <inery/chain/generated_transaction_object.hpp>
#include <inery/chain/transaction_object.hpp>
#include <inery/chain/reversible_block_object.hpp>
#include <inery/chain/genesis_intrinsics.hpp>
#include <inery/chain/whitelisted_intrinsics.hpp>
#include <inery/chain/database_header_object.hpp>

#include <inery/chain/protocol_feature_manager.hpp>
...
#include <inery/chain/thread_utils.hpp>
#include <inery/chain/platform_timer.hpp>
...
struct controller_impl {
   ...
   void pop_block() {
      ...
      if( !prev ) {
         INE_ASSERT( fork_db.root()->id == head->header.previous, block_validate_exception, "attempt to pop beyond last irreversible block" );
         prev = fork_db.root();
      }
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         INE_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
      }
   ...
   inline void set_activation_handler() {
      ...
      INE_ASSERT( res.second, misc_exception, "attempting to set activation handler twice" );
   }
   ...
   :rnh(),
    ...
    wasmif( cfg.wasm_runtime, cfg.inevmoc_tierup, db, cfg.state_dir, cfg.inevmoc_config ),
    resource_limits( db ),
    authorization( s, db ),
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( inery, inery, newaccount );
   SET_APP_HANDLER( inery, inery, setcode );
   SET_APP_HANDLER( inery, inery, setabi );
   SET_APP_HANDLER( inery, inery, updateauth );
   SET_APP_HANDLER( inery, inery, deleteauth );
   SET_APP_HANDLER( inery, inery, linkauth );
   SET_APP_HANDLER( inery, inery, unlinkauth );
/*
   SET_APP_HANDLER( inery, inery, postrecovery );
   SET_APP_HANDLER( inery, inery, passrecovery );
   SET_APP_HANDLER( inery, inery, vetorecovery );
*/

   SET_APP_HANDLER( inery, inery, canceldelay );
   }
   ...
   void log_irreversible() {
      INE_ASSERT( fork_db.root(), fork_database_exception, "fork database not properly initialized" );
      ...
      if( log_head ) {
         INE_ASSERT( root_id == blog.head_id(), fork_database_exception, "fork database root does not match block log head" );
      } else {
         INE_ASSERT( fork_db.root()->block_num == lib_num, fork_database_exception,
   ...
   void replay(std::function<bool()> shutdown) {
      ...
      if( start_block_num <= blog_head->block_num() ) {
         ...
         if( pending_head->block_num < head->block_num || head->block_num < fork_db.root()->block_num ) {
            ...
            INE_ASSERT( new_root, fork_database_exception, "unexpected error: could not find new LIB in fork database" );
   ...
   void startup(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      INE_ASSERT( snapshot, snapshot_exception, "No snapshot reader provided" );
      ...
      try {
         ...
         if( blog.head() ) {
            ...
            INE_ASSERT( lib_num > 0, snapshot_exception,
   ...
   void startup(std::function<bool()> shutdown, const genesis_state& genesis) {
      INE_ASSERT( db.revision() < 1, database_exception, "This version of controller::startup only works with a fresh state database." );
      const auto& genesis_chain_id = genesis.compute_chain_id();
      INE_ASSERT( genesis_chain_id == chain_id, chain_id_type_exception,
      ...
      if( blog.head() ) {
         INE_ASSERT( blog.first_block_num() == 1, block_log_exception,
   ...
   void startup(std::function<bool()> shutdown) {
      INE_ASSERT( db.revision() >= 1, database_exception, "This version of controller::startup does not work with a fresh state database." );
      INE_ASSERT( fork_db.head(), fork_database_exception, "No existing fork database despite existing chain state. Replay required." );

      uint32_t lib_num = fork_db.root()->block_num;
      auto first_block_num = blog.first_block_num();
      if( blog.head() ) {
         INE_ASSERT( first_block_num <= lib_num && lib_num <= blog.head()->block_num(),
                     block_log_exception,
   ...
   static auto validate_db_version( const chainbase::database& db ) {
      ...
      INE_ASSERT(header_idx.begin() != header_idx.end(), bad_database_version_exception,
   ...
   void init(std::function<bool()> shutdown) {
...

      {
         ...
         INE_ASSERT( state_chain_id == chain_id, chain_id_type_exception,
      ...
      INE_ASSERT( db.revision() >= head->block_num, fork_database_exception,
      ...
      if( read_mode == db_read_mode::IRREVERSIBLE ) {
         ...
         INE_ASSERT( itr == rbi.end() || itr->blocknum == lib_num + 1, reversible_blocks_exception,
         ...
         INE_ASSERT( head->block_num <= last_block_num, reversible_blocks_exception,
         ...
         if( ritr != rbi.rend()
            ...
            INE_ASSERT( rbitr != rbi.end(), reversible_blocks_exception, "pending head block not found in reversible blocks");
            auto rev_id = rbitr->get_block_id();
            INE_ASSERT( rev_id == pending_head->id,
   ...
   void read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      { /// load and upgrade the block header state
         ...
         INE_ASSERT( blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end,
                     block_log_exception,
      ...
      controller_index_set::walk_indices([this, &snapshot, &header]( auto utils ){
         ...
         if (std::is_same<value_t, global_property_object>::value) {
            ...
            if (std::clamp(header.version, v2::minimum_version, v2::maximum_version) == header.version ) {
               ...
               INE_ASSERT( genesis, snapshot_exception,
      ...
      INE_ASSERT( gpo.chain_id == chain_id, chain_id_type_exception,
   ...
   void create_native_account( const fc::time_point& initial_timestamp, account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            // The initial inery ABI value affects consensus; see  https://github.com/INERY/ine/issues/7794
            // TODO: This doesn't charge MEM; a fix requires a consensus upgrade.
            a.abi.resize(sizeof(inery_abi_bin));
            memcpy(a.abi.data(), inery_abi_bin, sizeof(inery_abi_bin));
         }
      });
      ...
      resource_limits.add_pending_mem_usage(name, mem_delta);
      resource_limits.verify_account_mem_usage(name);
   }
   ...
   int64_t remove_scheduled_transaction( const generated_transaction_object& gto ) {
      int64_t mem_delta = -(config::billable_size_v<generated_transaction_object> + gto.packed_trx.size());
      resource_limits.add_pending_mem_usage( gto.payer, mem_delta );
      // No need to verify_account_mem_usage since we are only reducing memory
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      INE_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      INE_ASSERT( !validating || explicit_billed_cpu_time, transaction_exception, "validating requires explicit billing" );
      ...
      int64_t trx_removal_mem_delta = remove_scheduled_transaction(gto);

      fc::datastream<const char*> ds( gtrx.packed_trx.data(), gtrx.packed_trx.size() );

      INE_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
      ...
      if( gtrx.expiration < self.pending_block_time() ) {
         ...
         trace->account_mem_delta = account_delta( gtrx.payer, trx_removal_mem_delta );
      ...
      if ( !subjective ) {
         ...
         if( !validating ) {
            ...
            INE_ASSERT( !explicit_billed_cpu_time || (cpu_time_to_bill_us == limited_cpu_time_to_bill_us),
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      INE_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   {
      INE_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
      ...
      try {
         ...
         if( !explicit_billed_cpu_time ) {
            fc::microseconds already_consumed_time( INE_PERCENT(sig_cpu_usage.count(), conf.sig_cpu_bill_pct) );
   ...
   {
      INE_ASSERT( !pending, block_validate_exception, "pending block already exists" );
      ...
      if (!self.skip_db_sessions(s)) {
         INE_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete )
      {
         ...
         if( new_protocol_feature_activations.size() > 0 ) {
            ...
            for( const auto& feature_digest : new_protocol_feature_activations ) {
               ...
               if( res.second ) {
                  ...
                  INE_ASSERT( !f.preactivation_required, protocol_feature_exception,
                              "attempted to activate protocol feature without prior required preactivation: ${digest}",
                              ("digest", feature_digest)
                  );
               } else {
                  INE_ASSERT( !res.first->second, block_validate_exception,
         ...
         INE_ASSERT( handled_all_preactivated_features, block_validate_exception,
         ...
         )
         {
            ...
            INE_ASSERT( gpo.proposed_schedule.version == pbhs.active_schedule_version + 1,
   ...
   void finalize_block()
   {
      INE_ASSERT( pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      INE_ASSERT( pending->_block_stage.contains<building_block>(), block_validate_exception, "already called finalize_block");
      ...
      uint64_t CPU_TARGET = INE_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {INE_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, config::maximum_elastic_resource_multiplier, {99, 100}, {1000, 999}}
      );
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         INE_ASSERT( pending->_block_stage.contains<completed_block>(), block_validate_exception,
         ...
         if( add_to_fork_db ) {
            ...
            head = fork_db.head();
            INE_ASSERT( bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
      ...
      for( auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr ) {
         ...
         switch( status ) {
            case protocol_feature_set::recognized_t::unrecognized:
               INE_THROW( protocol_feature_exception,
            ...
            break;
            default:
               INE_THROW( protocol_feature_exception, "unexpected recognized_t status" );
            break;
         }

         INE_ASSERT( currently_activated_protocol_features.find( f ) == currently_activated_protocol_features.end(),
         ...
         INE_ASSERT( pfs.validate_dependencies( f, dependency_checker ), protocol_feature_exception,
...
#define INE_REPORT(DESC,A,B) \
   if( A != B ) { \
      elog("${desc}: ${bv} != ${abv}", ("desc", DESC)("bv", A)("abv", B)); \
   }

      INE_REPORT( "timestamp", b.timestamp, ab.timestamp )
      INE_REPORT( "producer", b.producer, ab.producer )
      INE_REPORT( "confirmed", b.confirmed, ab.confirmed )
      INE_REPORT( "previous", b.previous, ab.previous )
      INE_REPORT( "transaction_mroot", b.transaction_mroot, ab.transaction_mroot )
      INE_REPORT( "action_mroot", b.action_mroot, ab.action_mroot )
      INE_REPORT( "schedule_version", b.schedule_version, ab.schedule_version )
      INE_REPORT( "new_producers", b.new_producers, ab.new_producers )
      INE_REPORT( "header_extensions", b.header_extensions, ab.header_extensions )
...
#undef INE_REPORT
      ...
      try {
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               INE_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
         ...
         if( producer_block_id != ab._id ) {
            ...
            INE_ASSERT( producer_block_id == ab._id, block_validate_exception, "Block ID does not match",
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      INE_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      INE_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block_header( b->previous );
      INE_ASSERT( prev, unlinkable_block_exception,
      ...
      return async_thread_pool( thread_pool.get_executor(), [b, prev, control=this]() {
         ...
         INE_ASSERT( b->transaction_mroot == trx_mroot, block_validate_exception,
                    ...
                    const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      INE_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         INE_ASSERT( b, block_validate_exception, "trying to push empty block" );
         INE_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
         ...
         if( s == controller::block_status::irreversible ) {
            ...
            INE_ASSERT( read_mode != db_read_mode::IRREVERSIBLE, block_validate_exception,
                            ...
                            const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         if( branches.second.size() > 0 ) {
            ...
            INE_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         INE_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         INE_ASSERT( !intersects, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         INE_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         INE_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         INE_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         INE_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      INE_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   switch( status ) {
      case protocol_feature_set::recognized_t::unrecognized:
         if( is_producing_block() ) {
            INE_THROW( subjective_block_production_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         } else {
            INE_THROW( protocol_feature_bad_block_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         }
      break;
      case protocol_feature_set::recognized_t::disabled:
      ...
      default:
         if( is_producing_block() ) {
            INE_THROW( subjective_block_production_exception, "unexpected recognized_t status" );
         } else {
            INE_THROW( protocol_feature_bad_block_exception, "unexpected recognized_t status" );
         }
      break;
   }
   ...
   // Even if they don't recognize the feature, the producer could change their nodine code to treat it like an
   ...
   INE_ASSERT( !is_protocol_feature_activated( feature_digest ),
               protocol_feature_exception,
   ...
   INE_ASSERT( std::find( pso.preactivated_protocol_features.begin(),
   ...
   INE_ASSERT( pfs.validate_dependencies( feature_digest, dependency_checker ),
               protocol_feature_exception,
...
void controller::start_block( block_timestamp_type when, uint16_t confirm_block_count )
{
   ...
   INE_ASSERT( !my->pending, block_validate_exception, "pending block already exists" );
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline,
   ...
   validate_db_available_size();
   INE_ASSERT( get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push transaction not allowed in irreversible mode" );
   INE_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
...
transaction_trace_ptr controller::push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline,
                                                              uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time )
{
   INE_ASSERT( get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push scheduled transaction not allowed in irreversible mode" );
   validate_db_available_size();
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      INE_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      INE_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   INE_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   INE_ASSERT( BOOST_LIKELY( id != block_id_type() ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   INE_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
bool controller::is_mem_billing_in_notify_allowed()const {
   return my->conf.disable_all_subjective_mitigations || !is_producing_block() || my->conf.allow_mem_billing_in_notify;
}
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   INE_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   INE_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   INE_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   INE_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::set_greylist_limit( uint32_t limit ) {
   INE_ASSERT( 0 < limit && limit <= chain::config::maximum_elastic_resource_multiplier,
               misc_exception,
...
void controller::add_to_mem_correction( account_name account, uint64_t mem_bytes ) {
   if( auto ptr = my->db.find<account_mem_correction_object, by_name>( account ) ) {
      my->db.modify<account_mem_correction_object>( *ptr, [&]( auto& rco ) {
         rco.mem_correction += mem_bytes;
      } );
   } else {
      my->db.create<account_mem_correction_object>( [&]( auto& rco ) {
         rco.name = account;
         rco.mem_correction = mem_bytes;
      } );
   }
...
void controller_impl::on_activation<builtin_protocol_feature_t::replace_deferred>() {
   const auto& indx = db.get_index<account_mem_correction_index, by_id>();
   for( auto itr = indx.begin(); itr != indx.end(); itr = indx.begin() ) {
      int64_t current_mem_usage = resource_limits.get_account_mem_usage( itr->name );
      int64_t mem_delta = -static_cast<int64_t>(itr->mem_correction);
      if( itr->mem_correction > static_cast<uint64_t>(current_mem_usage) ) {
         mem_delta = -current_mem_usage;
         elog( "account ${name} was to be reduced by ${adjust} bytes of MEM despite only using ${current} bytes of MEM",
               ("name", itr->name)("adjust", itr->mem_correction)("current", current_mem_usage) );
      }

      resource_limits.add_pending_mem_usage( itr->name, mem_delta );
      db.remove( *itr );
   }
}

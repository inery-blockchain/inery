...
#include <inery/chain/wasm_interface_private.hpp>
#include <inery/chain/wasm_inery_validation.hpp>
...
namespace inery { namespace chain {
   ...
   wasm_interface::wasm_interface(vm_type vm, bool inevmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const inevmoc::config& inevmoc_config)
     : my( new wasm_interface_impl(vm, inevmoc_tierup, d, data_dir, inevmoc_config) ) {}
   ...
   void wasm_interface::apply( const digest_type& code_hash, const uint8_t& vm_type, const uint8_t& vm_version, apply_context& context ) {
      ...
      if(my->inevmoc) {
         const chain::inevmoc::code_descriptor* cd = nullptr;
         try {
            cd = my->inevmoc->cc.get_descriptor_for_code(code_hash, vm_version);
         }
         catch(...) {
            //swallow errors here, if INE VM OC has gone in to the weeds we shouldn't bail: continue to try and run baseline
            //In the future, consider moving bits of INE VM that can fire exceptions and such out of this call path
            static bool once_is_enough;
            if(!once_is_enough)
               elog("INE VM OC has encountered an unexpected failure");
            once_is_enough = true;
         }
         if(cd) {
            my->inevmoc->exec.execute(*cd, my->inevmoc->mem, context);
            return;
         }
      }
#endif
   ...
   public:
      ...
      :context(ctx)
      {
         if( context.is_context_free() )
            INE_ASSERT( context_free, unaccessible_api, "only context free api's can be used in this context" );
      }
...
class context_free_api : public context_aware_api {
   public:
      ...
      :context_aware_api(ctx, true) {
         ...
         INE_ASSERT( context.is_context_free(), unaccessible_api, "this API may only be called from context_free apply" );
      }
...
class privileged_api : public context_aware_api {
   public:
      ...
      {
         INE_ASSERT( context.is_privileged(), unaccessible_api, "${code} does not have permission to call this API", ("code",context.get_receiver()) );
      }
      ...
      void activate_feature( int64_t feature_name ) {
         INE_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
      }
       ...
       * @param mem_bytes - the limit for ram bytes
       ...
       */
      void set_resource_limits( account_name account, int64_t mem_bytes, int64_t net_weight, int64_t cpu_weight) {
         INE_ASSERT(mem_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         INE_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         INE_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
         if( context.control.get_mutable_resource_limits_manager().set_account_limits(account, mem_bytes, net_weight, cpu_weight) ) {
            context.trx_context.validate_mem_usage.insert( account );
         }
      }

      void get_resource_limits( account_name account, int64_t& mem_bytes, int64_t& net_weight, int64_t& cpu_weight ) {
         context.control.get_resource_limits_manager().get_account_limits( account, mem_bytes, net_weight, cpu_weight);
      }

      int64_t set_proposed_producers_common( vector<producer_authority> && producers, bool validate_keys ) {
         INE_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
         INE_ASSERT( producers.size() > 0
         ...
         for (const auto& p: producers) {
            INE_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
            ...
            p.authority.visit([&p, num_supported_key_types, validate_keys](const auto& a) {
               ...
               for (const auto& kw: a.keys ) {
                  INE_ASSERT( kw.key.which() < num_supported_key_types, unactivated_key_type,
                              "Unactivated key type used in proposed producer schedule");

                  if( validate_keys ) {
                     INE_ASSERT( kw.key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
                  }
               ...
               INE_ASSERT( a.keys.size() == unique_keys.size(), wasm_execution_error, "producer schedule includes a duplicated key for ${account}", ("account", p.producer_name));
               INE_ASSERT( a.threshold > 0, wasm_execution_error, "producer schedule includes an authority with a threshold of 0 for ${account}", ("account", p.producer_name));
               INE_ASSERT( sum_weights >= a.threshold, wasm_execution_error, "producer schedule includes an unsatisfiable authority for ${account}", ("account", p.producer_name));
            });


            unique_producers.insert(p.producer_name);
         }
         INE_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
      ...
      int64_t set_proposed_producers_ex( uint64_t packed_producer_format, array_ptr<char> packed_producer_schedule, uint32_t datalen ) {
         if (packed_producer_format == 0) {
            ...
            INE_THROW(wasm_execution_error, "Producer schedule is in an unknown format!");
         }
      }
...
#pragma GCC diagnostic pop
      ...
      int32_t _inery_f32_trunc_i32s( float af ) {
         ...
         if (_inery_f32_ge(af, 2147483648.0f) || _inery_f32_lt(af, -2147483648.0f))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
      ...
      int32_t _inery_f64_trunc_i32s( double af ) {
         ...
         if (_inery_f64_ge(af, 2147483648.0) || _inery_f64_lt(af, -2147483648.0))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
      ...
      uint32_t _inery_f32_trunc_i32u( float af ) {
         ...
         if (_inery_f32_ge(af, 4294967296.0f) || _inery_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
      ...
      uint32_t _inery_f64_trunc_i32u( double af ) {
         ...
         if (_inery_f64_ge(af, 4294967296.0) || _inery_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
      ...
      int64_t _inery_f32_trunc_i64s( float af ) {
         ...
         if (_inery_f32_ge(af, 9223372036854775808.0f) || _inery_f32_lt(af, -9223372036854775808.0f))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
      ...
      int64_t _inery_f64_trunc_i64s( double af ) {
         ...
         if (_inery_f64_ge(af, 9223372036854775808.0) || _inery_f64_lt(af, -9223372036854775808.0))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");
      ...
      uint64_t _inery_f32_trunc_i64u( float af ) {
         ...
         if (_inery_f32_ge(af, 18446744073709551616.0f) || _inery_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
      ...
      uint64_t _inery_f64_trunc_i64u( double af ) {
         ...
         if (_inery_f64_ge(af, 18446744073709551616.0) || _inery_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( inery::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
...
class crypto_api : public context_aware_api {
   public:
      ...
      void assert_recover_key( const fc::sha256& digest,
         ...
         INE_ASSERT(s.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_signature_type,
           "Unactivated signature type used during assert_recover_key");
         INE_ASSERT(p.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_key_type,
           "Unactivated key type used when creating assert_recover_key");

         if(context.control.is_producing_block())
            INE_ASSERT(s.variable_size() <= context.control.configured_subjective_signature_length_limit(),
                       sig_variable_size_limit_exception, "signature variable length component size greater than subjective maximum");

         auto check = fc::crypto::public_key( s, digest, false );
         INE_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
      }
      ...
      int recover_key( const fc::sha256& digest,
         ...
         if (s.which() >= config::genesis_num_supported_key_types ) {
            INE_ASSERT(publen >= 33, wasm_execution_error,
      ...
      template<class Encoder> auto encode(char* data, uint32_t datalen) {
         Encoder e;
         const size_t bs = inery::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
      ...
      void assert_sha256(array_ptr<char> data, uint32_t datalen, const fc::sha256& hash_val) {
         ...
         INE_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         INE_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
...
public:
   ...
   void abort() {
      INE_ASSERT( false, abort_called, "abort() called");
   }

   // Kept as intrinsic rather than implementing on WASM side (using inery_assert_message and strlen) because strlen is faster on native side.
   void inery_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         const size_t sz = strnlen( msg, max_assert_message );
         std::string message( msg, sz );
         INE_THROW( inery_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void inery_assert_message( bool condition, array_ptr<const char> msg, uint32_t msg_len ) {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void inery_assert_code( bool condition, uint64_t error_code ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         if( error_code >= static_cast<uint64_t>(system_error_code::generic_system_error) ) {
            restricted_error_code_exception e( FC_LOG_MESSAGE(
                                                   error,
                                                   "inery_assert_code called with reserved error code: ${error_code}",
                                                   ("error_code", error_code)
            ) );
            e.error_code = static_cast<uint64_t>(system_error_code::contract_restricted_error_code);
            throw e;
         } else {
            inery_assert_code_exception e( FC_LOG_MESSAGE(
                                             error,
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         INE_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         return context.IDX.store( scope, table, account_name(payer), id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
         INE_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, uint32_t length) {
         INE_ASSERT((size_t)(std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
class transaction_api : public context_aware_api {
   public:
      ...
      void send_inline( array_ptr<char> data, uint32_t data_len ) {
         ...
         INE_ASSERT( data_len < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
...
class compiler_builtins : public context_aware_api {
   public:
      ...
      void __divti3(__int128& ret, uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb) {
         ...
         INE_ASSERT(rhs != 0, arithmetic_exception, "divide by zero");
...
/*
 * This api will be removed with fix for `ine #2561`
 */
...
REGISTER_INTRINSICS(context_free_system_api,
   (abort,                void()              )
   (inery_assert,         void(int, int)      )
   (inery_assert_message, void(int, int, int) )
   (inery_assert_code,    void(int, int64_t)  )
   (inery_exit,           void(int)           )
);
...
REGISTER_INJECTED_INTRINSICS(softfloat_api,
      (_inery_f32_add,       float(float, float)    )
      (_inery_f32_sub,       float(float, float)    )
      (_inery_f32_mul,       float(float, float)    )
      (_inery_f32_div,       float(float, float)    )
      (_inery_f32_min,       float(float, float)    )
      (_inery_f32_max,       float(float, float)    )
      (_inery_f32_copysign,  float(float, float)    )
      (_inery_f32_abs,       float(float)           )
      (_inery_f32_neg,       float(float)           )
      (_inery_f32_sqrt,      float(float)           )
      (_inery_f32_ceil,      float(float)           )
      (_inery_f32_floor,     float(float)           )
      (_inery_f32_trunc,     float(float)           )
      (_inery_f32_nearest,   float(float)           )
      (_inery_f32_eq,        int(float, float)      )
      (_inery_f32_ne,        int(float, float)      )
      (_inery_f32_lt,        int(float, float)      )
      (_inery_f32_le,        int(float, float)      )
      (_inery_f32_gt,        int(float, float)      )
      (_inery_f32_ge,        int(float, float)      )
      (_inery_f64_add,       double(double, double) )
      (_inery_f64_sub,       double(double, double) )
      (_inery_f64_mul,       double(double, double) )
      (_inery_f64_div,       double(double, double) )
      (_inery_f64_min,       double(double, double) )
      (_inery_f64_max,       double(double, double) )
      (_inery_f64_copysign,  double(double, double) )
      (_inery_f64_abs,       double(double)         )
      (_inery_f64_neg,       double(double)         )
      (_inery_f64_sqrt,      double(double)         )
      (_inery_f64_ceil,      double(double)         )
      (_inery_f64_floor,     double(double)         )
      (_inery_f64_trunc,     double(double)         )
      (_inery_f64_nearest,   double(double)         )
      (_inery_f64_eq,        int(double, double)    )
      (_inery_f64_ne,        int(double, double)    )
      (_inery_f64_lt,        int(double, double)    )
      (_inery_f64_le,        int(double, double)    )
      (_inery_f64_gt,        int(double, double)    )
      (_inery_f64_ge,        int(double, double)    )
      (_inery_f32_promote,    double(float)         )
      (_inery_f64_demote,     float(double)         )
      (_inery_f32_trunc_i32s, int(float)            )
      (_inery_f64_trunc_i32s, int(double)           )
      (_inery_f32_trunc_i32u, int(float)            )
      (_inery_f64_trunc_i32u, int(double)           )
      (_inery_f32_trunc_i64s, int64_t(float)        )
      (_inery_f64_trunc_i64s, int64_t(double)       )
      (_inery_f32_trunc_i64u, int64_t(float)        )
      (_inery_f64_trunc_i64u, int64_t(double)       )
      (_inery_i32_to_f32,     float(int32_t)        )
      (_inery_i64_to_f32,     float(int64_t)        )
      (_inery_ui32_to_f32,    float(int32_t)        )
      (_inery_ui64_to_f32,    float(int64_t)        )
      (_inery_i32_to_f64,     double(int32_t)       )
      (_inery_i64_to_f64,     double(int64_t)       )
      (_inery_ui32_to_f64,    double(int32_t)       )
      (_inery_ui64_to_f64,    double(int64_t)       )
);
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wabt")
      runtime = inery::chain::wasm_interface::vm_type::wabt;
   else if (s == "ine-vm")
      runtime = inery::chain::wasm_interface::vm_type::ine_vm;
   else if (s == "ine-vm-jit")
      runtime = inery::chain::wasm_interface::vm_type::ine_vm_jit;
   else if (s == "ine-vm-oc")
      runtime = inery::chain::wasm_interface::vm_type::ine_vm_oc;
   else

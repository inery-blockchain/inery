...
void apply_context::exec_one()
{
   ...
   try {
      try {
         ...
         if( !privileged && control.is_builtin_activated( builtin_protocol_feature_t::mem_restrictions ) ) {
            const size_t checktime_interval = 10;
            size_t counter = 0;
            bool   not_in_notify_context = (receiver == act->account);
            const auto end = _account_mem_deltas.end();
            for( auto itr = _account_mem_deltas.begin(); itr != end; ++itr, ++counter ) {
               if( counter == checktime_interval ) {
                  trx_context.checktime();
                  counter = 0;
               }
               if( itr->delta > 0 && itr->account != receiver ) {
                  INE_ASSERT( not_in_notify_context, unauthorized_mem_usage_increase,
                              "unprivileged contract cannot increase MEM usage of another account within a notify context: ${account}",
                              ("account", itr->account)
                  );
                  INE_ASSERT( has_authorization( itr->account ), unauthorized_mem_usage_increase,
                              "unprivileged contract cannot increase MEM usage of another account that has not authorized the action: ${account}",
                              ("account", itr->account)
                  );
               }
            }
   ...
   //    * and, the *receiver_account object itself cannot be removed because accounts cannot be deleted in INERY.
...
{
   trace.account_mem_deltas = std::move( _account_mem_deltas );
   _account_mem_deltas.clear();
...
void apply_context::exec()
{
   ...
   if( _cfa_inline_actions.size() > 0 || _inline_actions.size() > 0 ) {
      INE_ASSERT( recurse_depth < control.get_global_properties().configuration.max_inline_action_depth,
...
void apply_context::require_authorization( const account_name& account ) {
   ...
   INE_ASSERT( false, missing_auth_exception, "missing authority of ${account}", ("account",account));
}
...
void apply_context::require_authorization(const account_name& account,
  ...
  INE_ASSERT( false, missing_auth_exception, "missing authority of ${account}/${permission}",
...
void apply_context::execute_inline( action&& a ) {
   ...
   INE_ASSERT( code != nullptr, action_validate_exception,
   ...
   for( const auto& auth : a.authorization ) {
      ...
      INE_ASSERT( actor != nullptr, action_validate_exception,
                  "inline action's authorizing actor ${account} does not exist", ("account", auth.actor) );
      INE_ASSERT( control.get_authorization_manager().find_permission(auth) != nullptr, action_validate_exception,
   ...
   if( !privileged && control.is_producing_block() ) {
      ...
      INE_ASSERT( a.data.size() < std::min(chain_config.max_inline_action_size, control.get_max_nonprivileged_inline_action_size()),
      ...
      try {
         control.get_authorization_manager()
                .check_authorization( {a},
                                      {},
                                      {{receiver, config::inery_code_name}},
         ...
         if( disallow_send_to_self_bypass || !send_to_self ) {
            ...
            INE_THROW(subjective_block_production_exception, "Unexpected exception occurred validating inline action sent to self");
         }
      }
   }
...
void apply_context::execute_context_free_inline( action&& a ) {
   ...
   INE_ASSERT( a.authorization.size() == 0, action_validate_exception,
...
void apply_context::schedule_deferred_transaction( const uint128_t& sender_id, account_name payer, transaction&& trx, bool replace_existing ) {
   INE_ASSERT( trx.context_free_actions.size() == 0, cfa_inside_generated_tx, "context free actions are not currently allowed in generated transactions" );
   ...
   if( control.is_builtin_activated( builtin_protocol_feature_t::no_duplicate_deferred_id ) ) {
      ...
      if( exts.size() > 0 ) {
         ...
         INE_ASSERT( exts.size() == 1 && itr != exts.end(), invalid_transaction_extension,
         ...
         INE_ASSERT( context.sender == receiver, ill_formed_deferred_transaction_generation_context,
                     "deferred transaction generaction context contains mismatching sender",
                     ("expected", receiver)("actual", context.sender)
         );
         INE_ASSERT( context.sender_id == sender_id, ill_formed_deferred_transaction_generation_context,
                     "deferred transaction generaction context contains mismatching sender_id",
                     ("expected", sender_id)("actual", context.sender_id)
         );
         INE_ASSERT( context.sender_trx_id == trx_context.id, ill_formed_deferred_transaction_generation_context,
   ...
   bool mem_restrictions_activated = control.is_builtin_activated( builtin_protocol_feature_t::mem_restrictions );
   ...
   if( !control.skip_auth_check() && !privileged ) { // Do not need to check authorization if replayng irreversible block or if contract is privileged
      if( payer != receiver ) {
         if( mem_restrictions_activated ) {
            INE_ASSERT( receiver == act->account, action_validate_exception,
                        "cannot bill MEM usage of deferred transactions to another account within notify context"
            );
            INE_ASSERT( has_authorization( payer ), action_validate_exception,
                        "cannot bill MEM usage of deferred transaction to another account that has not authorized the action: ${payer}",
                        ("payer", payer)
            );
         } else {
      ...
      // It would make it possible to bill MEM to some unrelated account.
      ...
      try {
         ...
         if( disallow_send_to_self_bypass || !is_sending_only_to_self(receiver) ) {
            ...
            INE_THROW(subjective_block_production_exception, "Unexpected exception occurred validating sent deferred transaction consisting only of actions to self");
         }
      }
   }
   ...
   if ( auto ptr = db.find<generated_transaction_object,by_sender_id>(boost::make_tuple(receiver, sender_id)) ) {
      INE_ASSERT( replace_existing, deferred_tx_duplicate, "deferred transaction with the same sender_id and payer already exists" );

      bool replace_deferred_activated = control.is_builtin_activated(builtin_protocol_feature_t::replace_deferred);

      INE_ASSERT( replace_deferred_activated || !control.is_producing_block()
                     || control.all_subjective_mitigations_disabled(),
                  subjective_block_production_exception,
                  "Replacing a deferred transaction is temporarily disabled." );

      uint64_t orig_trx_mem_bytes = config::billable_size_v<generated_transaction_object> + ptr->packed_trx.size();
      if( replace_deferred_activated ) {
         add_mem_usage( ptr->payer, -static_cast<int64_t>( orig_trx_mem_bytes ) );
      } else {
         control.add_to_mem_correction( ptr->payer, orig_trx_mem_bytes );
      }
   ...
   INE_ASSERT( mem_restrictions_activated
               || control.is_mem_billing_in_notify_allowed()
               || (receiver == act->account) || (receiver == payer) || privileged,
               subjective_block_production_exception,
               "Cannot charge MEM to other accounts during notify."
   );
   add_mem_usage( payer, (config::billable_size_v<generated_transaction_object> + trx_size) );
}
...
bool apply_context::cancel_deferred_transaction( const uint128_t& sender_id, account_name sender ) {
   ...
   if ( gto ) {
      add_mem_usage( gto->payer, -(config::billable_size_v<generated_transaction_object> + gto->packed_trx.size()) );
...
void apply_context::update_db_usage( const account_name& payer, int64_t delta ) {
   if( delta > 0 ) {
      if( !(privileged || payer == account_name(receiver)
               || control.is_builtin_activated( builtin_protocol_feature_t::mem_restrictions ) ) )
      {
         INE_ASSERT( control.is_mem_billing_in_notify_allowed() || (receiver == act->account),
                     subjective_block_production_exception, "Cannot charge MEM to other accounts during notify." );
         require_authorization( payer );
      }
   }
   add_mem_usage(payer, delta);
}
...
int apply_context::get_action( uint32_t type, uint32_t index, char* buffer, size_t buffer_size )const
{
   ...
   INE_ASSERT(act_ptr, action_not_found_exception, "action is not found" );
...
//   require_write_lock( scope );
   ...
   INE_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );
...
void apply_context::db_update_i64( int iterator, account_name payer, const char* buffer, size_t buffer_size ) {
   ...
   INE_ASSERT( table_obj.code == receiver, table_access_violation, "db access violation" );
...
int apply_context::db_previous_i64( int iterator, uint64_t& primary ) {
   ...
   if( iterator < -1 ) // is end iterator
   {
      ...
      INE_ASSERT( tab, invalid_table_iterator, "not a valid end iterator" );
...
void apply_context::add_mem_usage( account_name account, int64_t mem_delta ) {
   trx_context.add_mem_usage( account, mem_delta );

   auto p = _account_mem_deltas.emplace( account, mem_delta );
   if( !p.second ) {
      p.first->delta += mem_delta;
   }
}

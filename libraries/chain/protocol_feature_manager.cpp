...
Also corrects the MEM usage of accounts affected by the replace deferred transaction bug.
*/
            {}
         } )
...
Removes the restriction on inery::linkauth for non-native actions named one of the five special action names:
...
Forward inery::setcode actions to the WebAssembly code deployed on the inery account.
*/
            {}
         } )
...
*/
         ...
         (  builtin_protocol_feature_t::mem_restrictions, builtin_protocol_feature_spec{
            "MEM_RESTRICTIONS",
            fc::variant("1812fdb5096fd854a4958eb9d53b43219d114de0e858ce00255bd46569ad2c68").as<digest_type>(),
            // SHA256 hash of the raw message below within the comment delimiters (do not modify message below).
/*
Builtin protocol feature: MEM_RESTRICTIONS

Modifies the restrictions on operations within actions that increase MEM usage of accounts other than the receiver.

An unprivileged contract responding to a notification:
is not allowed to schedule a deferred transaction in which the MEM costs are paid by an account other than the receiver;
but is allowed to execute database operations that increase MEM usage of an account other than the receiver as long as
the action's net effect on MEM usage for the account is to not increase it.
...
but is allowed to execute database operations that increase MEM usage of an account other than the receiver as long as
either the account authorized the action or the action's net effect on MEM usage for the account is to not increase it.
*/
            {}
         } )
...
*/
   ...
   const char* builtin_protocol_feature_codename( builtin_protocol_feature_t codename ) {
      ...
      INE_ASSERT( itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
   ...
   ,_type( feature_type )
   {
      switch( feature_type ) {
         ...
         default:
         {
            INE_THROW( protocol_feature_validation_exception,
   ...
   {
      INE_ASSERT( builtin_feature, protocol_feature_exception, "not a builtin protocol feature" );
   ...
   const protocol_feature& protocol_feature_set::get_protocol_feature( const digest_type& feature_digest )const {
      ...
      INE_ASSERT( itr != _recognized_protocol_features.end(), protocol_feature_exception,
   ...
   const protocol_feature& protocol_feature_set::add_feature( const builtin_protocol_feature& f ) {
      ...
      INE_ASSERT( builtin_itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
      ...
      if( indx < _recognized_builtin_protocol_features.size() ) {
         INE_ASSERT( _recognized_builtin_protocol_features[indx] == _recognized_protocol_features.end(),
      ...
      if( expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size() ) {
         ...
         for( const auto& builtin_codename : missing_builtins ) {
            ...
            INE_ASSERT( itr != builtin_protocol_feature_codenames.end(),
                        protocol_feature_exception,
                        "Unexpected error"
            );
      ...
      INE_ASSERT( res.second, protocol_feature_exception,
   ...
   void protocol_feature_manager::init( chainbase::database& db ) {
      INE_ASSERT( !is_initialized(), protocol_feature_exception, "cannot initialize protocol_feature_manager twice" );
   ...
   const protocol_feature* protocol_feature_manager::const_iterator::get_pointer()const {
      //INE_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot dereference singular iterator" );
      //INE_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot dereference end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator++() {
      INE_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot increment singular iterator" );
      INE_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot increment end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator--() {
      INE_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot decrement singular iterator" );
      if( _index == end_index ) {
         INE_ASSERT( _pfm->_activated_protocol_features.size() > 0,
                     protocol_feature_iterator_exception,
   ...
   {
      INE_ASSERT( is_initialized(), protocol_feature_exception, "protocol_feature_manager is not yet initialized" );

      auto itr = _protocol_feature_set.find( feature_digest );

      INE_ASSERT( itr != _protocol_feature_set.end(), protocol_feature_exception,
                  "unrecognized protocol feature digest: ${digest}", ("digest", feature_digest) );

      if( _activated_protocol_features.size() > 0 ) {
         const auto& last = _activated_protocol_features.back();
         INE_ASSERT( last.activation_block_num <= current_block_num,
                     protocol_feature_exception,
      ...
      INE_ASSERT( itr->builtin_feature,
                  protocol_feature_exception,
      ...
      INE_ASSERT( indx < _builtin_protocol_features.size(), protocol_feature_exception,
      ...
      INE_ASSERT( _builtin_protocol_features[indx].activation_block_num == builtin_protocol_feature_entry::not_active,
                  protocol_feature_exception,

...
#include <inery/chain/transaction_metadata.hpp>
...
namespace inery { namespace chain { namespace resource_limits {
...
void elastic_limit_parameters::validate()const {
   ...
   INE_ASSERT( periods > 0, resource_limit_exception, "elastic limit parameter 'periods' cannot be zero" );
   INE_ASSERT( contract_rate.denominator > 0, resource_limit_exception, "elastic limit parameter 'contract_rate' is not a well-defined ratio" );
   INE_ASSERT( expand_rate.denominator > 0, resource_limit_exception, "elastic limit parameter 'expand_rate' is not a well-defined ratio" );
}
...
void resource_limits_manager::add_transaction_usage(const flat_set<account_name>& accounts, uint64_t cpu_usage, uint64_t net_usage, uint32_t time_slot ) {
   ...
   for( const auto& a : accounts ) {
      ...
      verify_account_mem_usage(a);

      _db.modify( usage, [&]( auto& bu ){
          bu.net_usage.add( net_usage, time_slot, config.account_net_usage_average_window );
          bu.cpu_usage.add( cpu_usage, time_slot, config.account_cpu_usage_average_window );
          bu.mem_usage+= config::transaction_id_net_usage;
      });
      ...
      if( cpu_weight >= 0 && state.total_cpu_weight > 0 ) {
         ...
         INE_ASSERT( cpu_used_in_window <= max_user_use_in_window,
      ...
      if( net_weight >= 0 && state.total_net_weight > 0) {
         ...
         INE_ASSERT( net_used_in_window <= max_user_use_in_window,
   ...
   INE_ASSERT( state.pending_cpu_usage <= config.cpu_limit_parameters.max, block_resource_exhausted, "Block has insufficient cpu resources" );
   INE_ASSERT( state.pending_net_usage <= config.net_limit_parameters.max, block_resource_exhausted, "Block has insufficient net resources" );
}

void resource_limits_manager::add_pending_mem_usage( const account_name account, int64_t mem_delta ) {
   if (mem_delta == 0) {
      return;
   }
   ...
   INE_ASSERT( mem_delta <= 0 || UINT64_MAX - usage.mem_usage >= (uint64_t)mem_delta, transaction_exception,
              "Mem usage delta would overflow UINT64_MAX");
   INE_ASSERT(mem_delta >= 0 || usage.mem_usage >= (uint64_t)(-mem_delta), transaction_exception,
              "Mem usage delta would underflow UINT64_MAX");

   _db.modify( usage, [&]( auto& u ) {
     u.mem_usage += mem_delta;
   });
}

void resource_limits_manager::verify_account_mem_usage( const account_name account )const {
   int64_t mem_bytes; int64_t net_weight; int64_t cpu_weight;
   get_account_limits( account, mem_bytes, net_weight, cpu_weight );
   const auto& usage  = _db.get<resource_usage_object,by_owner>( account );

   if( mem_bytes >= 0 ) {
      INE_ASSERT( usage.mem_usage <= static_cast<uint64_t>(mem_bytes), mem_usage_exceeded,
                  "account ${account} has insufficient mem; needs ${needs} bytes has ${available} bytes",
                  ("account", account)("needs",usage.mem_usage)("available",mem_bytes)              );
   }
}

int64_t resource_limits_manager::get_account_mem_usage( const account_name& name )const {
   return _db.get<resource_usage_object,by_owner>( name ).mem_usage;
}


bool resource_limits_manager::set_account_limits( const account_name& account, int64_t mem_bytes, int64_t net_weight, int64_t cpu_weight) {
   //const auto& usage = _db.get<resource_usage_object,by_owner>( account );
   /*
   ...
   auto find_or_create_pending_limits = [&]() -> const resource_limits_object& {
      ...
      if (pending_limits == nullptr) {
         ...
         return _db.create<resource_limits_object>([&](resource_limits_object& pending_limits){
            ...
            pending_limits.mem_bytes = limits.mem_bytes;
   ...
   if( mem_bytes >= 0 ) {
      ...
      decreased_limit = ( (limits.mem_bytes < 0) || (mem_bytes < limits.mem_bytes) );

      /*
      if( limits.mem_bytes < 0 ) {
         INE_ASSERT(mem_bytes >= usage.mem_usage, wasm_execution_error, "converting unlimited account would result in overcommitment [commit=${c}, desired limit=${l}]", ("c", usage.mem_usage)("l", mem_bytes));
      } else {
         INE_ASSERT(mem_bytes >= usage.mem_usage, wasm_execution_error, "attempting to release committed ram resources [commit=${c}, desired limit=${l}]", ("c", usage.mem_usage)("l", mem_bytes));
      }
      */
   }

   _db.modify( limits, [&]( resource_limits_object& pending_limits ){
      pending_limits.mem_bytes = mem_bytes;
...
void resource_limits_manager::get_account_limits( const account_name& account, int64_t& mem_bytes, int64_t& net_weight, int64_t& cpu_weight ) const {
   const auto* pending_buo = _db.find<resource_limits_object,by_owner>( boost::make_tuple(true, account) );
   if (pending_buo) {
      mem_bytes  = pending_buo->mem_bytes;
...
void resource_limits_manager::process_account_limit_updates() {
   ...
   auto update_state_and_value = [](uint64_t &total, int64_t &value, int64_t pending_value, const char* debug_which) -> void {
      if (value > 0) {
         INE_ASSERT(total >= static_cast<uint64_t>(value), rate_limiting_state_inconsistent, "underflow when reverting old value to ${which}", ("which", debug_which));
         total -= value;
      }

      if (pending_value > 0) {
         INE_ASSERT(UINT64_MAX - total >= static_cast<uint64_t>(pending_value), rate_limiting_state_inconsistent, "overflow when applying new value to ${which}", ("which", debug_which));
         total += pending_value;
      }
   ...
   _db.modify(state, [&](resource_limits_state_object& rso){
      while(!by_owner_index.empty()) {
         ...
         _db.modify(actual_entry, [&](resource_limits_object& rlo){
            update_state_and_value(rso.total_mem_bytes,  rlo.mem_bytes,  itr->mem_bytes, "mem_bytes");

...
namespace inery { namespace chain {
   ...
   void transaction_context::disallow_transaction_extensions( const char* error_msg )const {
      if( control.is_producing_block() ) {
         INE_THROW( subjective_block_production_exception, error_msg );
      } else {
         INE_THROW( disallowed_transaction_extensions_bad_block_exception, error_msg );
      }
   }

   void transaction_context::init(uint64_t initial_net_usage)
   {
      INE_ASSERT( !is_initialized, transaction_exception, "cannot initialize twice" );
      ...
      validate_mem_usage.reserve( bill_to_accounts.size() );
   ...
   void transaction_context::exec() {
      INE_ASSERT( is_initialized, transaction_exception, "must first initialize" );
   ...
   void transaction_context::finalize() {
      ...
      auto& rl = control.get_mutable_resource_limits_manager();
      for( auto a : validate_mem_usage ) {
         rl.verify_account_mem_usage( a );
      }
      ...
      if (!control.skip_trx_checks()) {
         if( BOOST_UNLIKELY(net_usage > eager_net_limit) ) {
            if ( net_limit_due_to_block ) {
               INE_THROW( block_net_usage_exceeded,
               ...
               INE_THROW( greylist_net_usage_exceeded,
                          "greylisted transaction net usage is too high: ${net_usage} > ${net_limit}",
                          ("net_usage", net_usage)("net_limit", eager_net_limit) );
            } else {
               INE_THROW( tx_net_usage_exceeded,
   ...
   void transaction_context::checktime()const {
      ...
      if( explicit_billed_cpu_time || deadline_exception_code == deadline_exception::code_value ) {
         INE_THROW( deadline_exception, "deadline exceeded ${billing_timer}us",
                     ("billing_timer", now - pseudo_start)("now", now)("deadline", _deadline)("start", start) );
      } else if( deadline_exception_code == block_cpu_usage_exceeded::code_value ) {
         INE_THROW( block_cpu_usage_exceeded,
   ...
   void transaction_context::validate_cpu_usage_to_bill( int64_t billed_us, int64_t account_cpu_limit, bool check_minimum )const {
      if (!control.skip_trx_checks()) {
         if( check_minimum ) {
            const auto& cfg = control.get_global_properties().configuration;
            INE_ASSERT( billed_us >= cfg.min_transaction_cpu_usage, transaction_exception,
   ...
   void transaction_context::validate_account_cpu_usage( int64_t billed_us, int64_t account_cpu_limit, bool estimate )const {
      if( (billed_us > 0) && !control.skip_trx_checks() ) {
         ...
         if( !cpu_limited_by_account && (billing_timer_exception_code == block_cpu_usage_exceeded::code_value) ) {
            INE_ASSERT( billed_us <= objective_duration_limit.count(),
            ...
            if( cpu_limit_due_to_greylist && cpu_limited_by_account ) {
               INE_ASSERT( billed_us <= account_cpu_limit,
               ...
               INE_ASSERT( billed_us <= cpu_limit,
   ...
   void transaction_context::add_mem_usage( account_name account, int64_t mem_delta ) {
      auto& rl = control.get_mutable_resource_limits_manager();
      rl.add_pending_mem_usage( account, mem_delta );
      if( mem_delta > 0 ) {
         validate_mem_usage.insert( account );
      }
   }
   ...
   std::tuple<int64_t, int64_t, bool, bool> transaction_context::max_bandwidth_billed_accounts_can_pay( bool force_elastic_limits ) const{
      ...
      INE_ASSERT( (!force_elastic_limits && control.is_producing_block()) || (!greylisted_cpu && !greylisted_net),
   ...
   action_trace& transaction_context::get_action_trace( uint32_t action_ordinal ) {
      INE_ASSERT( 0 < action_ordinal && action_ordinal <= trace->action_traces.size() ,
   ...
   void transaction_context::schedule_transaction() {
      ...
      int64_t mem_delta = (config::billable_size_v<generated_transaction_object> + trx_size);
      add_mem_usage( cgto.payer, mem_delta );
      trace->account_mem_delta = account_delta( cgto.payer, mem_delta );
   }
   ...
   void transaction_context::validate_referenced_accounts( const transaction& trx, bool enforce_actor_whitelist_blacklist )const {
      ...
      for( const auto& a : trx.context_free_actions ) {
         ...
         INE_ASSERT( code != nullptr, transaction_exception,
                     "action's code account '${account}' does not exist", ("account", a.account) );
         INE_ASSERT( a.authorization.size() == 0, transaction_exception,
      ...
      for( const auto& a : trx.actions ) {
         ...
         for( const auto& auth : a.authorization ) {
            ...
            INE_ASSERT( actor  != nullptr, transaction_exception,
                        "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
            INE_ASSERT( auth_manager.find_permission(auth) != nullptr, transaction_exception,
      ...
      INE_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );

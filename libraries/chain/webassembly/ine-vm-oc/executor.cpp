#include <inery/chain/webassembly/ine-vm-oc/executor.hpp>
#include <inery/chain/webassembly/ine-vm-oc/code_cache.hpp>
#include <inery/chain/webassembly/ine-vm-oc/memory.hpp>
#include <inery/chain/webassembly/ine-vm-oc/intrinsic_mapping.hpp>
...
#error INE VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, INEVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, INEVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic INEVMOC_INTRINSIC_INIT_PRIORITY("inevmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&ine_vm_oc_grow_memory,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("inevmoc_internal.grow_memory"))).value()
);

//This is effectively overriding the inery_exit intrinsic in wasm_interface
static void inery_exit(int32_t code) {
   siglongjmp(*ine_vm_oc_get_jmp_buf(), INEVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
static intrinsic inery_exit_intrinsic("env.inery_exit", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32}), (void*)&inery_exit,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("env.inery_exit"))).value()
);

static void throw_internal_exception(const char* const s) {
   *reinterpret_cast<std::exception_ptr*>(ine_vm_oc_get_exception_ptr()) = std::make_exception_ptr(wasm_execution_error(FC_LOG_MESSAGE(error, s)));
   siglongjmp(*ine_vm_oc_get_jmp_buf(), INEVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_INEVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function INEVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(#module "." #name))).value() \
   ); \
	void name()

DEFINE_INEVMOC_TRAP_INTRINSIC(inevmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_INEVMOC_TRAP_INTRINSIC(inevmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_INEVMOC_TRAP_INTRINSIC(inevmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_INEVMOC_TRAP_INTRINSIC(inevmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_INEVMOC_TRAP_INTRINSIC(inevmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. INE VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, const memory& mem, apply_context& context) {
   ...
   cb->current_call_depth_remaining = inery::chain::wasm_constraints::maximum_call_depth+2;
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      case 0:
         ...
         break;
      //case 1: clean inery_exit
      case INEVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case INEVMOC_EXIT_SEGV:
         INE_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case INEVMOC_EXIT_EXCEPTION: //exception

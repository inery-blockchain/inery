...
std::tuple<bool, inevmoc_message, std::vector<wrapped_fd>> read_message_with_fds(boost::asio::local::datagram_protocol::socket& s) {
   return read_message_with_fds(s.native_handle());
}

std::tuple<bool, inevmoc_message, std::vector<wrapped_fd>> read_message_with_fds(int fd) {
   char buff[max_message_size];

   struct msghdr msg = {};
   struct cmsghdr* cmsg;

   inevmoc_message message;
...
bool write_message_with_fds(boost::asio::local::datagram_protocol::socket& s, const inevmoc_message& message, const std::vector<wrapped_fd>& fds) {
   return write_message_with_fds(s.native_handle(), message, fds);
}

bool write_message_with_fds(int fd_to_send_to, const inevmoc_message& message, const std::vector<wrapped_fd>& fds) {
   struct msghdr msg = {};
   struct cmsghdr* cmsg;

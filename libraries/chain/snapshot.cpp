...
void variant_snapshot_reader::validate() const {
   INE_ASSERT(snapshot.is_object(), snapshot_validation_exception,
         "Variant snapshot is not an object");
   const fc::variant_object& o = snapshot.get_object();

   INE_ASSERT(o.contains("version"), snapshot_validation_exception,
         "Variant snapshot has no version");

   const auto& version = o["version"];
   INE_ASSERT(version.is_integer(), snapshot_validation_exception,
         "Variant snapshot version is not an integer");

   INE_ASSERT(version.as_uint64() == (uint64_t)current_snapshot_version, snapshot_validation_exception,
   ...
   INE_ASSERT(o.contains("sections"), snapshot_validation_exception,
         "Variant snapshot has no sections");

   const auto& sections = o["sections"];
   INE_ASSERT(sections.is_array(), snapshot_validation_exception, "Variant snapshot sections is not an array");

   const auto& section_array = sections.get_array();
   for( const auto& section: section_array ) {
      INE_ASSERT(section.is_object(), snapshot_validation_exception, "Variant snapshot section is not an object");

      const auto& so = section.get_object();
      INE_ASSERT(so.contains("name"), snapshot_validation_exception,
            "Variant snapshot section has no name");

      INE_ASSERT(so["name"].is_string(), snapshot_validation_exception,
                 "Variant snapshot section name is not a string");

      INE_ASSERT(so.contains("rows"), snapshot_validation_exception,
                 "Variant snapshot section has no rows");

      INE_ASSERT(so["rows"].is_array(), snapshot_validation_exception,
...
void variant_snapshot_reader::set_section( const string& section_name ) {
   ...
   INE_THROW(snapshot_exception, "Variant snapshot has no section named ${n}", ("n", section_name));
}
...
{
   INE_ASSERT(section_pos == std::streampos(-1), snapshot_exception, "Attempting to write a new section without closing the previous section");
...
void istream_snapshot_reader::validate() const {
   ...
   try {
      ...
      INE_ASSERT(actual_totem == expected_totem, snapshot_exception,
      ...
      INE_ASSERT(actual_version == expected_version, snapshot_exception,
...
void istream_snapshot_reader::set_section( const string& section_name ) {
   ...
   INE_THROW(snapshot_exception, "Binary snapshot has no section named ${n}", ("n", section_name));
}

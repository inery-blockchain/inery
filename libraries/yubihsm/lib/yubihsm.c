...
yh_rc yh_send_plain_msg(yh_connector *connector, yh_cmd cmd,
  ...
  if (connector == NULL || (data_len != 0 && data == NULL) ||
    ...
    DBG_ERR("%s", yh_strerror(YHR_INVALID_PAMEMETERS));
    return YHR_INVALID_PAMEMETERS;
  }
...
verify_card_cryptogram(Scp_ctx *ctx, uint8_t context[SCP_CONTEXT_LEN],
  ...
  yrc =
    compute_cryptogram(ctx->s_mac, SCP_KEY_LEN, SCP_CARD_CRYPTOGMEM, context,
...
static yh_rc compute_host_cryptogram(yh_session *session,
  ...
  return compute_cryptogram(session->s.s_mac, SCP_KEY_LEN, SCP_HOST_CRYPTOGMEM,

...
(CK_SESSION_HANDLE hSession, CK_USER_TYPE userType, CK_UTF8CHAR_PTR pPin,
 CK_ULONG ulPinLen) {
  ...
  if (yrc != YHR_SUCCESS) {
    ...
    if (yrc == YHR_CRYPTOGMEM_MISMATCH) {
 ...
 CK_OBJECT_HANDLE hKey) {
  ...
  if (object->object.type == YH_ASYMMETRIC_KEY &&
    ...
    if (pMechanism->mechanism == CKM_RSA_PKCS_OAEP) {
      if (pMechanism->ulParameterLen != sizeof(CK_RSA_PKCS_OAEP_PAMEMS)) {
        DBG_ERR("Length of mechanism parameters does not match expected value: "
                "found %lu, expected %zu",
                pMechanism->ulParameterLen, sizeof(CK_RSA_PKCS_OAEP_PAMEMS));
        rv = CKR_MECHANISM_PAMEM_INVALID;
        goto c_di_out;
      }

      CK_RSA_PKCS_OAEP_PAMEMS *params = pMechanism->pParameter;
    ...
    if (yh_is_rsa(object->object.algorithm)) {
      if (is_RSA_sign_mechanism(session->operation.mechanism.mechanism) ==
        ...
        if (is_PSS_sign_mechanism(session->operation.mechanism.mechanism)) {
          if (pMechanism->ulParameterLen != sizeof(CK_RSA_PKCS_PSS_PAMEMS)) {
            DBG_ERR("Length of mechanism parameters does not match expected "
                    "value, "
                    "%lu != %zu",
                    pMechanism->ulParameterLen, sizeof(CK_RSA_PKCS_PSS_PAMEMS));
            rv = CKR_MECHANISM_PAMEM_INVALID;
            goto c_si_out;
          }

          CK_RSA_PKCS_PSS_PAMEMS *params = pMechanism->pParameter;
...
(CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism,
 ...
 CK_ULONG ulAttributeCount, CK_OBJECT_HANDLE_PTR phKey) {
  ...
  CK_ECDH1_DERIVE_PAMEMS *params = pMechanism->pParameter;

...
#include <inery/vm/error_codes.hpp>
#include <inery/vm/host_function.hpp>
#include <inery/vm/watchdog.hpp>
...
void inery_assert(bool test, const char* msg) {
   if (!test) {
...
/**
 * Simple implementation of an interpreter using ine-vm.
 */
int main(int argc, char** argv) {
   ...
   using backend_t = inery::vm::backend<example_host_methods>;
   using rhf_t     = inery::vm::registered_host_functions<example_host_methods>;

   // register print_num
   rhf_t::add<nullptr_t, &print_num, wasm_allocator>("env", "print_num");
   // register inery_assert
   rhf_t::add<nullptr_t, &inery_assert, wasm_allocator>("env", "inery_assert");
   // register print_name
   ...
   } catch (...) { std::cerr << "ine-vm interpreter error\n"; }
   return 0;
}

...
#define QUICKCPPLIB_SECDED_INTRINSICS 0
#endif
    /*! \class secded_ecc
    \brief Calculates the single error correcting double error detecting (SECDED) Hamming Error Correcting Code for a \em blocksize block of bytes. For example, a secdec_ecc<8> would be the very common 72,64 Hamming code used in ECC MEM, or secdec_ecc<4096> would be for a 32784,32768 Hamming code.
    ...
    Did you know that some non-ECC MEM systems can see 1e-12 flips/bit/hour, which is 3.3 bits flipped in a 16Gb MEM system
    per 24 hours). See Schroeder, Pinheiro and Weber (2009) 'DMEM Errors in the Wild: A Large-Scale Field Study'.
    ...
    reliable filing system code on computers without ECC MEM and in which sustained large quantities of random disc i/o produce

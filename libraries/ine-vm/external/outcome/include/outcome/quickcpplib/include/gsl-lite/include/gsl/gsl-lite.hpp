...
#if gsl_ELIDE_CONTRACT_EXPECTS
# define gsl_EXPECTS_UNUSED_PAMEM( x )  /* Make param unnamed if Expects elided */
#else
# define gsl_EXPECTS_UNUSED_PAMEM( x )  x
#endif
...
#if gsl_HAVE( NULLPTR )
    gsl_api gsl_constexpr14 span( std::nullptr_t, index_type gsl_EXPECTS_UNUSED_PAMEM( size_in ) )
        : first_( nullptr )
        , last_ ( nullptr )
    {
        Expects( size_in == 0 );

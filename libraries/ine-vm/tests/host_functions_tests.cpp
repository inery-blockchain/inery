...
// wasm hex
/* Code used to generate test, compile with inery-cpp v1.6.2 with minor manual edits to remove unneeded imports
 * extern "C" {
...
struct init_backend {
   ...
   using backend_t = inery::vm::backend<Host, Impl>;
   using rhf_t     = inery::vm::registered_host_functions<Host>;
   wasm_allocator wa;
...
BACKEND_TEST_CASE( "Test C-style host function system", "[C-style_host_functions_tests]") { 
   wasm_allocator wa;
   using backend_t = inery::vm::backend<nullptr_t, TestType>;
   using rhf_t     = inery::vm::registered_host_functions<nullptr_t>;

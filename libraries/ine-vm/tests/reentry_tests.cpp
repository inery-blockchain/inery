...
BACKEND_TEST_CASE("test reentry", "[reentry]") {
   ...
   struct test_runner {
      inery::vm::backend<test_runner, TestType>& bkend;
      ...
      void inery_assert(uint32_t, uint32_t) {}
      void* memset(void*, int, int) { return 0; }
   };

   using rhf_t     = inery::vm::registered_host_functions<test_runner>;
   using backend_t = inery::vm::backend<test_runner, TestType>;
   wasm_allocator wa;
   ...
   rhf_t::template add<test_runner, &test_runner::inery_assert, wasm_allocator>("env", "inery_assert");

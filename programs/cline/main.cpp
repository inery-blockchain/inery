/**
  @defgroup ineclienttool

  @section intro Introduction to cline

  `cline` is a command line tool that interfaces with the REST api exposed by @ref nodine. In order to use `cline` you will need to
  have a local copy of `nodine` running and configured to load the 'inery::chain_api_plugin'.

   cline contains documentation for all of its commands. For a list of all commands known to cline, simply run it with no arguments:
```
$ ./cline
Command Line Interface to INERY Client
Usage: programs/cline/cline [OPTIONS] SUBCOMMAND
...
Subcommands:
  ...
  account                     Create a new account on the blockchain (assumes system contract does not restrict MEM usage)

$ ./cline create account
...
#include <inery/chain/wast_to_wasm.hpp>
#include <inery/chain/trace.hpp>
#include <inery/chain_plugin/chain_plugin.hpp>
#include <inery/chain/contract_types.hpp>

#include <inery/version/version.hpp>
...
using namespace inery::client::help;
using namespace inery::client::http;
using namespace inery::client::localize;
using namespace inery::client::config;
using namespace boost::filesystem;
using auth_type = fc::static_variant<public_key_type, permission_level>;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define INEC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
//copy pasta from kined's main.cpp
...
string default_wallet_url = "unix://" + (determine_home_directory() / "inery-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
inery::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<inery::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return inery::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
...
inery::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<inery::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, packed_transaction::compression_type compression = packed_transaction::compression_type::none ) {
   ...
   if (trx.signatures.size() == 0) { // #5445 can't change txn content if already signed
      ...
      } INE_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
auto abi_serializer_resolver = [](const name& account) -> fc::optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<inery::chain_apis::read_only::get_abi_results>();
...
fc::variant json_from_file_or_string(const string& file_or_str, fc::json::parse_type ptype = fc::json::parse_type::legacy_parser)
{
   ...
   if ( !regex_search(file_or_str, r) && fc::is_regular_file(file_or_str) ) {
      ...
      } INE_RETHROW_EXCEPTIONS(json_parse_exception, "Fail to parse JSON from file: ${file}", ("file", file_or_str));

   } else {
      try {
         return fc::json::from_string(file_or_str, ptype);
      } INE_RETHROW_EXCEPTIONS(json_parse_exception, "Fail to parse JSON from string: ${string}", ("string", file_or_str));
   }
}
...
void send_actions(std::vector<chain::action>&& actions, packed_transaction::compression_type compression = packed_transaction::compression_type::none ) {
   ...
   if (tx_json_save_file.length()) {
      ...
      INEC_ASSERT(!out.fail(), "ERROR: Failed to create file \"${p}\"", ("p", tx_json_save_file));
   }
...
chain::action create_buymem(const name& creator, const name& newaccount, const asset& quantity) {
   ...
   return create_action(get_account_permissions(tx_permission, {creator,config::active_name}),
                        config::system_account_name, N(buyresources), act_payload);
}
...
chain::action create_open(const string& contract, const name& owner, symbol sym, const name& mem_payer) {
   auto open_ = fc::mutable_variant_object
      ("owner", owner)
      ("symbol", sym)
      ("mem_payer", mem_payer);
    return action {
      get_account_permissions(tx_permission, {mem_payer, config::active_name}),
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } INE_RETHROW_EXCEPTIONS(authority_type_exception, "Invalid authority format '${data}'",
                            ("data", fc::json::to_string(authority_var, fc::time_point::maximum())))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "INE") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } INE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      INE_ASSERT( inery::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( account_name code, const string& s ) {
   static map< pair<account_name, inery::chain::symbol_code>, inery::chain::symbol> cache;
   auto a = asset::from_string( s );
   inery::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<inery::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.emplace( make_pair( code, sym ), result.max_supply.get_symbol() );
         it = p.first;
      } else {
         INE_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      INE_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( N(inery.token), s );
}
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::inery_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::inery_code_name))));

      add_standard_transaction_options(permissions, "account@active");

      permissions->callback([this] {
         INEC_ASSERT( !(add_code && remove_code), "ERROR: Either --add-code or --remove-code can be set" );
         INEC_ASSERT( (add_code ^ remove_code) || !authority_json_or_file.empty(), "ERROR: authority should be specified unless add or remove code permission" );
         ...
         if ( need_parent || need_auth ) {
            ...
            auto res = json.as<inery::chain_apis::read_only::get_account_results>();
            ...
            if ( need_auth ) {
               ...
               auto code_name = config::inery_code_name;
...
void ensure_kined_running(CLI::App* app) {
    if (no_auto_kined)
        return;
    // get, version, net, convert do not require kined
    ...
    // This extra check is necessary when running cline like this: ./cline ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if cline and kined are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kined").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child kine(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (kine.running()) {
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->callback([this] {
         ...
         } INE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
...
struct create_account_subcommand {
   ...
   string stake_net;
   string stake_cpu;
   uint32_t buy_mem_bytes_in_kbytes = 0;
   uint32_t buy_mem_bytes = 0;
   string buy_mem_ine;
   bool transfer = false;
   bool simple = false;
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      auto createAccount = actionRoot->add_subcommand(
                              ...
                              (simple ? localized("Create a new account on the blockchain (assumes system contract does not restrict MEM usage)")
      ...
      if (!simple) {
         ...
         // createAccount->add_option("--buy-mem-kbytes", buy_mem_bytes_in_kbytes,
         //                           (localized("The amount of MEM bytes to purchase for the new account in kibibytes (KiB)")));
         createAccount->add_option("--buy-mem-bytes", buy_mem_bytes,
                                   (localized("The amount of MEM bytes to purchase for the new account in bytes")));
         // createAccount->add_option("--buy-mem", buy_mem_ine,
         //                           (localized("The amount of MEM bytes to purchase for the new account in tokens")));
      ...
      createAccount->callback([this] {
            ...
            if( owner_key_str.find('@') != string::npos ) {
               ...
               } INE_RETHROW_EXCEPTIONS( explained_exception, "Invalid owner permission level: ${permission}", ("permission", owner_key_str) )
            } else {
               try {
                  owner = public_key_type(owner_key_str);
               } INE_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str) );
            }
            ...
            if( active_key_str.empty() ) {
               ...
               } INE_RETHROW_EXCEPTIONS( explained_exception, "Invalid active permission level: ${permission}", ("permission", active_key_str) )
            } else {
               try {
                  active = public_key_type(active_key_str);
               } INE_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str) );
            }

            auto create = create_newaccount(name(creator), name(account_name), owner, active);
            if (!simple) {
               // INEC_ASSERT( buy_mem_ine.size() || buy_mem_bytes_in_kbytes || buy_mem_bytes, "ERROR: One of --buy-mem, --buy-mem-kbytes or --buy-mem-bytes should have non-zero value" );
               // INEC_ASSERT( !buy_mem_bytes_in_kbytes || !buy_mem_bytes, "ERROR: --buy-mem-kbytes and --buy-mem-bytes cannot be set at the same time" );
               INEC_ASSERT(buy_mem_bytes==1048576,"Wrong mem capacity");
               // action buyresources = !buy_mem_ine.empty() ? create_buymem(name(creator), name(account_name), to_asset(buy_mem_ine))
               //    : create_buymembytes(name(creator), name(account_name), (buy_mem_bytes_in_kbytes) ? (buy_mem_bytes_in_kbytes * 1024) : buy_mem_bytes);
               
               action buyresources = create_buymembytes(name(creator), name(account_name), buy_mem_bytes);

               auto net = to_asset(stake_net);
               auto cpu = to_asset(stake_cpu);
               INEC_ASSERT(net.get_amount()==10000,"Wrong net capacity");
               INEC_ASSERT(cpu.get_amount()==10000,"Wrong cpu capacity");
               if ( net.get_amount() == 10000  || cpu.get_amount() == 10000 ) {
                  action delegate = create_delegate( name(creator), name(account_name), net, cpu, transfer);
                  send_actions( { create, buyresources, delegate } );
               } else {
                  send_actions( { create, buyresources } );
               }
            } else {
               send_actions( { create } );
            }
...
struct vote_producer_proxy_subcommand {
   ...
   vote_producer_proxy_subcommand(CLI::App* actionRoot) {
      ...
      vote_proxy->callback([this] {
         ...
         send_actions({create_action(accountPermissions, config::system_account_name, N(makeprod), act_payload)});
      });
   }
};
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so cline can still work with old buggy nodine versions
                               // Change to voter.value when cline no longer needs to support nodine versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<inery::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when cline no longer needs to support nodine versions older than 1.5.0
            ...
            INE_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<inery::name> prods;
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         ...
         auto result = rawResult.as<inery::chain_apis::read_only::get_producers_result>();
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->callback([&] {
         ...
         } INE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
//       // delegate_bandwidth->add_option("--buyresources", buy_mem_amount, localized("The amount of tokens to buyresources"));
//       // delegate_bandwidth->add_option("--buy-mem-bytes", buy_mem_bytes, localized("The amount of MEM to buy in number of bytes"));
...
//          INEC_ASSERT( !(buy_mem_amount.size()) || !buy_mem_bytes, "ERROR: --buyresources and --buy-mem-bytes cannot be set at the same time" );
//          if (buy_mem_amount.size()) {
//             acts.push_back( create_buymem(name(from_str), name(receiver_str), to_asset(buy_mem_amount)) );
//          } else if (buy_mem_bytes) {
//             acts.push_back( create_buymembytes(name(from_str), name(receiver_str), buy_mem_bytes) );
//          }
...
//                                ("code", "inery")("scope", "inery")("table", "namebids")
//                                ("lower_bound", name(newname).to_uint64_t())
//                                ("upper_bound", name(newname).to_uint64_t() + 1)
//                                // Less than ideal upper_bound usage preserved so cline can still work with old buggy nodine versions
//                                // Change to newname.value when cline no longer needs to support nodine versions older than 1.5.0
//                                ("limit", 1));
//          if ( print_json ) {
//             std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
//             return;
//          }
//          auto result = rawResult.as<inery::chain_apis::read_only::get_table_rows_result>();
//          // Condition in if statement below can simply be res.rows.empty() when cline no longer needs to support nodine versions older than 1.5.0
...
struct buymem_subcommand {
   ...
   buymem_subcommand(CLI::App* actionRoot) {
      auto buyresources = actionRoot->add_subcommand("buyresources", localized("Buy MEM"));
      buyresources->add_option("payer", from_str, localized("The account paying for MEM"))->required();
      buyresources->add_option("receiver", receiver_str, localized("The account receiving bought MEM"))->required();
      buyresources->add_option("amount", amount, localized("The amount of tokens to pay for MEM, or number of bytes/kibibytes of MEM if --bytes/--kbytes is set"))->required();
      buyresources->add_flag("--kbytes,-k", kbytes, localized("buyresources in number of kibibytes (KiB)"));
      buyresources->add_flag("--bytes,-b", bytes, localized("buyresources in number of bytes"));
      add_standard_transaction_options(buyresources, "payer@active");
      buyresources->callback([this] {
         INEC_ASSERT( !kbytes || !bytes, "ERROR: --kbytes and --bytes cannot be set at the same time" );
         if (kbytes || bytes) {
            send_actions( { create_buymembytes(name(from_str), name(receiver_str), fc::to_uint64(amount) * ((kbytes) ? 1024ull : 1ull)) } );
         } else {
            send_actions( { create_buymem(name(from_str), name(receiver_str), to_asset(amount)) } );
         }
      });
   }
};

struct sellres_subcommand {
   string from_str;
   string receiver_str;
   uint64_t amount;

   sellres_subcommand(CLI::App* actionRoot) {
      auto sellres = actionRoot->add_subcommand("sellres", localized("Sell MEM"));
      sellres->add_option("account", receiver_str, localized("The account to receive tokens for sold MEM"))->required();
      sellres->add_option("bytes", amount, localized("Number of MEM bytes to sell"))->required();
      add_standard_transaction_options(sellres, "account@active");

      sellres->callback([this] {
            fc::variant act_payload = fc::mutable_variant_object()
               ("account", receiver_str)
               ("bytes", amount);
            auto accountPermissions = get_account_permissions(tx_permission, {name(receiver_str), config::active_name});
            send_actions({create_action(accountPermissions, config::system_account_name, N(sellres), act_payload)});
         });
   }
};
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      unordered_map<name, inery::chain_apis::permission> cache;
      ...
      std::cout << "memory: " << std::endl
                << indent << "quota: " << std::setw(15) << to_pretty_net(res.mem_quota) << "  used: " << std::setw(15) << to_pretty_net(res.mem_usage) << std::endl << std::endl;
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodine responding to the request is different than core symbol built into cline
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'inery::refund' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = inery::client::http::create_http_context();
   wallet_url = default_wallet_url;

   CLI::App app{"Command Line Interface to INERY Client"};
   ...
   app.add_flag( "--no-auto-" + string(key_store_executable_name), no_auto_kined, localized("don't automatically launch a ${k} if one is not currently running", ("k", key_store_executable_name)));
   app.parse_complete_callback([&app]{ ensure_kined_running(&app);});
   ...
   pack_transaction->callback([&] {
      ...
      if( pack_action_data_flag ) {
         ...
         } INE_RETHROW_EXCEPTIONS( transaction_type_exception, "Invalid transaction format: '${data}'",
         ...
         } INE_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to convert transaction, --pack-action-data likely needed" )
      }
   });
   ...
   unpack_transaction->callback([&] {
      ...
      } INE_RETHROW_EXCEPTIONS( transaction_type_exception, "Invalid packed transaction format: '${data}'",
   ...
   pack_action_data->callback([&] {
      ...
      } INE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse unpacked action data JSON")
   ...
   unpack_action_data->callback([&] {
      INE_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
   ...
   getCode->callback([&] {
      ...
      try {
         ...
         //see if this is an old nodine that doesn't support get_raw_code_and_abi
   ...
   getTable->add_flag("--show-payer", show_payer, localized("show MEM payer"));
   ...
   getAccounts->callback([&] {
      ...
      } INE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        INE_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        INE_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );
      ...
      if (!duplicate) {
         ...
         } INE_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
         if ( shouldSend ) {
   ...
   contractSubcommand->callback([&] {
      if(!contract_clear) INE_ASSERT( !contractPath.empty(), contract_exception, " contract-dir is null ", ("f", contractPath) );
      shouldSend = false;
      set_code_callback();
      set_abi_callback();
      if (actions.size()) {
   ...
   transfer->add_flag("--pay-mem-to-open", pay_mem, localized("Pay mem to open recipient's token balance row"));
   ...
   createWallet->callback([&wallet_name, &password_file, &print_console] {
      INEC_ASSERT( !password_file.empty() ^ print_console, "ERROR: Either indicate a file using \"--file\" or pass \"--to-console\"" );
      INEC_ASSERT( password_file.empty() || !std::ofstream(password_file.c_str()).fail(), "ERROR: Failed to create file in specified path" );
   ...
   importWallet->callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         INE_THROW(private_key_type_exception, "Invalid private key")
      }
   ...
   removeKeyWallet->callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         INE_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   auto stopKined = wallet->add_subcommand("stop", localized("Stop ${k}.", ("k", key_store_executable_name)));
   stopKined->callback([] {
      const auto& v = call(wallet_url, kined_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kined responds with empty object
   ...
   sign->callback([&] {
      ...
      INEC_ASSERT( str_private_key.empty() || str_public_key.empty(), "ERROR: Either -k/--private-key or --public-key or none of them can be set" );
      ...
      if( str_public_key.size() > 0 ) {
         ...
         } INE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", str_public_key))
         ...
         } INE_RETHROW_EXCEPTIONS(private_key_type_exception, "Invalid private key")
   ...
   propose_action->callback([&] {
      ...
      } INE_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } INE_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            INE_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, N(inery.msig), N(propose), variant_to_bin( N(inery.msig), N(propose), args ) }});
   });
   ...
   review->callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so cline can still work with old buggy nodine versions
                                 // Change to name(proposal_name).value when cline no longer needs to support nodine versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when cline no longer needs to support nodine versions older than 1.5.0
      ...
      std::map<inery::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<inery::name>() != a.first ) {
                  continue;
               }
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(inery.msig), name(action), variant_to_bin( N(inery.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(inery.msig), N(invalidate), variant_to_bin( N(inery.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            INE_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, N(inery.msig), N(cancel), variant_to_bin( N(inery.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            INE_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, N(inery.msig), N(exec), variant_to_bin( N(inery.msig), N(exec), args ) }});
      }
   );
   ...
   wrap_exec->add_option("executer", executer, localized("Account executing the transaction and paying for the deferred transaction MEM"))->required();
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send inery.system contract action to the blockchain."));
   ...
   auto makeProd = system->add_subcommand("makeprod", localized("Producer can validate blocks"));
   makeProd->require_subcommand();
   auto voteProxy = vote_producer_proxy_subcommand(makeProd);
   auto makeProds = vote_producers_subcommand(makeProd);
   auto approveProducer = approve_producer_subcommand(makeProd);
   auto unapproveProducer = unapprove_producer_subcommand(makeProd);
   ...
   auto buyresources = buymem_subcommand(system);
   auto sellres = sellres_subcommand(system);

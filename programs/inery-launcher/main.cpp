...
#include <net/if.h>
#include <inery/chain/genesis_state.hpp>
...
using namespace inery::launcher::config;
...
void
ined_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string inery_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), inery_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   string ined_extra_args;
   std::map<uint,string> specific_nodine_args;
   std::map<uint,string> specific_nodine_installation_paths;
   testnet_def network;
   string gelf_endpoint;
   ...
   inery::chain::genesis_state genesis_from_file;

   void assign_name (ined_def &node, bool is_bios);
   ...
   void prep_remote_config_dir (ined_def &node, host_def *host);
   void launch (ined_def &node, string &gts);
   void kill (launch_modes mode, string sig_opt);
   static string get_node_num(uint16_t node_num);
   pair<host_def, ined_def> find_node(uint16_t node_num);
   vector<pair<host_def, ined_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    (node_executable_name, bpo::value<string>(&ined_extra_args), ("forward " + string(node_executable_name) + " command line argument(s) to each instance of " + string(node_executable_name) + ", enclose arg(s) in quotes").c_str())
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodine_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodine_installation_paths);
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &ined : binding.instances) {
          ined.host = binding.host_name;
          ined.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(ined.p2p_port);

          aliases.push_back (ined.name);
        }
      }
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodine_args[num] = specific_args[i];
    }
  }

  char* erd_env_var = getenv ("INERY_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable INERY_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (ined_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.inery_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      ined_def ined;
      assign_name(ined, i == 0);
      aliases.push_back(ined.name);
      ined.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(ined));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->inery_home =
          (local_id.contains (lhost->host_name) || servers.default_inery_home.empty()) ?
          erd : servers.default_inery_home;
        host_ndx++;
      } // ph_count == 0

      ined_def ined;
      assign_name(ined, do_bios);
      ined.has_db = false;
      ...
      aliases.push_back(ined.name);
      ined.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(ined));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  ined_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->inery_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->inery_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->inery_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->inery_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->inery_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = inery::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = inery::mongo_db_plugin\n";
  }
  cfg << "plugin = inery::net_plugin\n";
  cfg << "plugin = inery::chain_api_plugin\n"
...
void
launcher_def::prep_remote_config_dir (ined_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->inery_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->inery_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->inery_home;

  cmd = "cd " + host->inery_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->inery_home
...
void
launcher_def::launch (ined_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodine_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodine_installation_paths.count(node_num)) {
        install_path = specific_nodine_installation_paths[node_num] + "/";
     }
  }
  string inedcmd = install_path + "programs/nodine/" + string(node_executable_name) + " ";
  if (skip_transaction_signatures) {
    inedcmd += "--skip-transaction-signatures ";
  }
  if (!ined_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +inery::mongo_db_plugin");
       string args = std::regex_replace (ined_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       inedcmd += args + " ";
    }
    else {
       inedcmd += ined_extra_args + " ";
    }
  }
  if (instance.name != "bios" && !specific_nodine_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodine_args.count(node_num)) {
        inedcmd += specific_nodine_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    inedcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  inedcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  inedcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    inedcmd += " --genesis-timestamp " + gts;
  }
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->inery_home + "; nohup " + inedcmd + " > "
    ...
    string cmd = "cd " + host->inery_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else if (!instance.node->dont_start) {
    cerr << "spawning child, " << inedcmd << endl;

    bp::child c(inedcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << inedcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
    ...
    cerr << "not spawning child, " << inedcmd << endl;
...
#if 0
void
launcher_def::kill_instance(ined_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, ined_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, ined_def>> node_list;
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.inery_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const ined_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/inery-tn_bounce.sh " + ined_extra_args;
      if (node_num != "bios" && !specific_nodine_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodine_args.count(node_num_i)) {
            cmd += " " + specific_nodine_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "INERY_HOME", host.inery_home }, { "INERY_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/inery-tn_down.sh ";
      do_command(host, node.name,
                 { { "INERY_HOME", host.inery_home }, { "INERY_NODE", node_num }, { "INERY_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/inery-tn_roll.sh ";
      do_command(*host, host_name, { { "INERY_HOME", host->inery_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the inery-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the inery-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the inery-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << inery::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(inery_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (inery_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_inery_home) (ssh) )

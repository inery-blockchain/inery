...
#include <inery/chain/block_header.hpp>
...
class state_history_log {
 ...
 public:
   ...
   void read_header(state_history_log_header& header, bool assert_version = true) {
      ...
      INE_ASSERT(!ds.remaining(), chain::plugin_exception, "state_history_log_header_serial_size mismatch");
      if (assert_version)
         INE_ASSERT(is_ship(header.magic) && is_ship_supported_version(header.magic), chain::plugin_exception,
   ...
   void write_entry(const state_history_log_header& header, const chain::block_id_type& prev_id, F write_payload) {
      ...
      INE_ASSERT(_begin_block == _end_block || block_num <= _end_block, chain::plugin_exception,
                 "missed a block in ${name}.log", ("name", name));

      if (_begin_block != _end_block && block_num > _begin_block) {
         if (block_num == _end_block) {
            INE_ASSERT(prev_id == last_block_id, chain::plugin_exception, "missed a fork change in ${name}.log",
                       ("name", name));
         } else {
            state_history_log_header prev;
            get_entry(block_num - 1, prev);
            INE_ASSERT(prev_id == prev.block_id, chain::plugin_exception, "missed a fork change in ${name}.log",
                       ("name", name));
         }
      }
      ...
      INE_ASSERT(end == pos + state_history_log_header_serial_size + header.payload_size, chain::plugin_exception,
   ...
   fc::cfile& get_entry(uint32_t block_num, state_history_log_header& header) {
      INE_ASSERT(block_num >= _begin_block && block_num < _end_block, chain::plugin_exception,
 ...
 private:
   ...
   void recover_blocks(uint64_t size) {
      ...
      log.flush();
      INE_ASSERT(get_last_block(pos), chain::plugin_exception, "recover ${name}.log failed", ("name", name));
   }
   ...
   void open_log() {
      ...
      if (size >= state_history_log_header_serial_size) {
         ...
         log.seek(0);
         read_header(header, false);
         INE_ASSERT(is_ship(header.magic) && is_ship_supported_version(header.magic) &&
         ...
         INE_ASSERT(!size, chain::plugin_exception, "corrupt ${name}.log (5)", ("name", name));
   ...
   void open_index() {
      ...
      while (pos < size) {
         state_history_log_header header;
         INE_ASSERT(pos + state_history_log_header_serial_size <= size, chain::plugin_exception,
         ...
         INE_ASSERT(suffix == pos, chain::plugin_exception, "corrupt ${name}.log (8)", ("name", name));
...
FC_REFLECT(inery::state_history_log_header, (magic)(block_id)(payload_size))

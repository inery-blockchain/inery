#include <inery/history_plugin/history_plugin.hpp>
#include <inery/history_plugin/account_control_history_object.hpp>
#include <inery/history_plugin/public_key_history_object.hpp>
...
CHAINBASE_SET_INDEX_TYPE(inery::account_history_object, inery::account_history_index)
CHAINBASE_SET_INDEX_TYPE(inery::action_history_object, inery::action_history_index)
...
namespace inery {
      ...
      try {
         if( options.count( "filter-on" )) {
            ...
            for( auto& s : fo ) {
               if( s == "*" || s == "\"*\"" ) {
                  ...
                  wlog( "--filter-on * enabled. This can fill shared_mem, causing nodine to stop." );
                  break;
               }
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               INE_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-on", ("s", s));
               filter_entry fe{inery::chain::name(v[0]), inery::chain::name(v[1]), inery::chain::name(v[2])};
               INE_ASSERT( fe.receiver.to_uint64_t(), fc::invalid_arg_exception,
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               INE_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{inery::chain::name(v[0]), inery::chain::name(v[1]), inery::chain::name(v[2])};
               INE_ASSERT( fe.receiver.to_uint64_t(), fc::invalid_arg_exception,
   ...
   namespace history_apis {
      read_only::get_actions_result read_only::get_actions( const read_only::get_actions_params& params )const {
        ...
        INE_ASSERT( end >= start, chain::plugin_exception, "end position is earlier than start position" );
      ...
      read_only::get_transaction_result read_only::get_transaction( const read_only::get_transaction_params& p )const {
         ...
         } INE_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", p.id))
         ...
         if( !in_history && !p.block_num_hint ) {
            INE_THROW(tx_not_found, "Transaction ${id} not found in history and no block hint was given", ("id",p.id));
         }
         ...
         if( in_history ) {
            ...
            if (!found) {
               INE_THROW(tx_not_found, "Transaction ${id} not found in history or in block number ${n}", ("id",p.id)("n", *p.block_num_hint));
            }
         }

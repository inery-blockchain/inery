...
namespace {
   ...
   bool is_onblock(const chain::transaction_trace_ptr& p) {
      ...
      if (act.account != inery::chain::config::system_account_name || act.name != N(onblock) ||
          act.authorization.size() != 1)
         return false;
      const auto& auth = act.authorization[0];
      return auth.actor == inery::chain::config::system_account_name &&
             auth.permission == inery::chain::config::active_name;
   }
      ...
      void commit_block(const chain::block_state_ptr& bsp ) {
         ...
         if (!updated.empty() || !deleted.empty() || rollback_required) {
            ...
            for (const auto& up: updated) {
               ...
               INE_ASSERT(source_itr != permission_by_owner.end(), chain::plugin_exception, "chain data is missing");

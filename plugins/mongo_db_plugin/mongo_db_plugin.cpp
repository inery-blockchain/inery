#include <inery/mongo_db_plugin/mongo_db_plugin.hpp>
#include <inery/mongo_db_plugin/bson.hpp>
...
void mongo_db_plugin_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
      ...
      //
      // Allow logging traces if node is a producer for testing purposes, so a single nodine can do both for testing.
      //
         ...
         if(account) {
            ...
            if( view.find( "abi" ) != view.end()) {
               ...
               if( n == chain::config::system_account_name ) {
                  // redefine inery setabi.abi from bytes to abi_def
                           ...
                           abis.add_specialized_unpack_pack( "abi_def",
                                 std::make_pair<abi_serializer::unpack_function, abi_serializer::pack_function>(
                                       []( fc::datastream<const char*>& stream, bool is_array, bool is_optional, const abi_serializer::yield_function_t& yield ) {
                                          INE_ASSERT( !is_array && !is_optional, chain::mongo_db_exception, "unexpected abi_def");
                                          chain::bytes temp;
                                       ...
                                       []( const fc::variant& var, fc::datastream<char*>& ds, bool is_array, bool is_optional, const abi_serializer::yield_function_t& yield ) {
                                          INE_ASSERT( false, chain::mongo_db_exception, "never called" );
                                       }
                                 ) );
                        }
                     }
...
void mongo_db_plugin_impl::_process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   ...
   try {
      ...
      if( !_trans.update_one( make_document( kvp( "trx_id", trx_id_str ) ),
         ...
         INE_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", trx_id) );
      }
   } catch( ... ) {
      ...
      try {
         ...
         try {
            if( !_trans_traces.insert_one( trans_traces_doc.view() ) ) {
               INE_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert trans ${id}", ("id", t->id) );
            }
         } catch( ... ) {
   ...
   if( write_atraces ) {
      try {
         if( !bulk_action_traces.execute() ) {
            INE_ASSERT( false, chain::mongo_db_insert_fail,
      ...
      try {
         if( update_blocks_via_block_num ) {
            if( !_block_states.update_one( make_document( kvp( "block_num", b_int32{static_cast<int32_t>(block_num)} ) ),
               ...
               INE_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${num}", ("num", block_num) );
            }
         } else {
            if( !_block_states.update_one( make_document( kvp( "block_id", block_id_str ) ),
                                           make_document( kvp( "$set", block_state_doc.view() ) ), update_opts ) ) {
               INE_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block_state ${bid}", ("bid", block_id) );
            }
         }
      } catch( ... ) {
      ...
      try {
         if( update_blocks_via_block_num ) {
            if( !_blocks.update_one( make_document( kvp( "block_num", b_int32{static_cast<int32_t>(block_num)} ) ),
               ...
               INE_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${num}", ("num", block_num) );
            }
         } else {
            if( !_blocks.update_one( make_document( kvp( "block_id", block_id_str ) ),
                                     make_document( kvp( "$set", block_doc.view() ) ), update_opts ) ) {
               INE_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert block ${bid}", ("bid", block_id) );
            }
         }
      } catch( ... ) {
...
void mongo_db_plugin_impl::_process_irreversible_block(const chain::block_state_ptr& bs)
{
   ...
   if( store_transactions ) {
      ...
      if( transactions_in_block ) {
         try {
            if( !bulk.execute() ) {
               INE_ASSERT( false, chain::mongo_db_insert_fail, "Bulk transaction insert failed for block: ${bid}", ("bid", block_id) );
            }
         } catch( ... ) {
...
void mongo_db_plugin_impl::remove_pub_keys( const account_name& name, const permission_name& permission )
{
   ...
   try {
      auto result = _pub_keys.delete_many( make_document( kvp( "account", name.to_string()),
                                                         kvp( "permission", permission.to_string())));
      if( !result ) {
         INE_ASSERT( false, chain::mongo_db_update_fail,
...
void create_account( mongocxx::collection& accounts, const name& name, std::chrono::milliseconds& now ) {
   ...
   try {
      if( !accounts.update_one( make_document( kvp( "name", name_str )), update.view(), update_opts )) {
         INE_ASSERT( false, chain::mongo_db_update_fail, "Failed to insert account ${n}", ("n", name));
      }
   } catch (...) {
            ...
            try {
               ...
               try {
                  if( !_accounts.update_one( make_document( kvp( "_id", account->view()["_id"].get_oid())),
                                             update_from.view())) {
                     INE_ASSERT( false, chain::mongo_db_update_fail, "Failed to udpdate account ${n}", ("n", setabi.account));
                  }
               } catch( ... ) {
      ...
      if( accounts.estimated_document_count() == 0 ) {
         ...
         try {
            if( !accounts.insert_one( doc.view())) {
               INE_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert account ${n}",
         ...
         try {
            ...
            //   1. enableSharding database (default to INE)
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The target queue size between nodine and MongoDB plugin thread.")
         ...
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'INE' is used if not specified in URI."
               " Example: mongodb://127.0.0.1:27017/INE")
         ...
         ("mongodb-filter-on", bpo::value<vector<string>>()->composing(),
          "Track actions which match receiver:action:actor. Receiver, Action, & Actor may be blank to include all. i.e. inery:: or :transfer:  Use * or leave unspecified to include all.")
      ...
      if( options.count( "mongodb-uri" )) {
         ...
         if( options.at( "replay-blockchain" ).as<bool>() || options.at( "hard-replay-blockchain" ).as<bool>() || options.at( "delete-all-blocks" ).as<bool>() ) {
            if( options.at( "mongodb-wipe" ).as<bool>()) {
               ...
               INE_ASSERT( false, chain::plugin_config_exception, "--mongodb-wipe required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks"
                                 " --mongodb-wipe will remove all INE collections from mongodb." );
            }
         }

         if( options.count( "abi-serializer-max-time-ms") == 0 ) {
            INE_ASSERT(false, chain::plugin_config_exception, "--abi-serializer-max-time-ms required as default value not appropriate for parsing full blocks");
         }
         ...
         if( options.count( "mongodb-abi-cache-size" )) {
            ...
            INE_ASSERT( my->abi_cache_size > 0, chain::plugin_config_exception, "mongodb-abi-cache-size > 0 required" );
         }
         ...
         if( options.count( "mongodb-filter-on" )) {
            ...
            for( auto& s : fo ) {
               ...
               INE_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-on", ("s", s));
               filter_entry fe{inery::chain::name(v[0]), inery::chain::name(v[1]), inery::chain::name(v[2])};
         ...
         if( options.count( "mongodb-filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               INE_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --mongodb-filter-out", ("s", s));
               filter_entry fe{inery::chain::name(v[0]), inery::chain::name(v[1]), inery::chain::name(v[2])};
         ...
         INE_ASSERT( chain_plug, chain::missing_chain_plugin_exception, ""  );
         ...
         wlog( "inery::mongo_db_plugin configured, but no --mongodb-uri specified." );
